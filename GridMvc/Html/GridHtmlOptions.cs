...
namespace GridMvc.Html
{
	...
	public class GridHtmlOptions<T> : IGridHtmlOptions<T> where T : class
	{
		...
		{
			string _gridName = "grid";
			if (_source.RenderOptions != null) {
				if (!string.IsNullOrEmpty(_source.RenderOptions.GridName)) {
					_gridName = _source.RenderOptions.GridName;
				}
			}
			return WithPaging(pageSize, maxDisplayedItems, string.Format("{0}-pager", _gridName));
		}
		...
		{
			if(_source.RenderOptions == null) {
				_source.RenderOptions = new GridRenderOptions();
			}
